...
func GetPageBooksByPrice(w http.ResponseWriter, r *http.Request) {
	...
	////解析模板文件
	//t := template.Must(template.ParseFiles("src/bookstore0612/views/index.html"))
	////执行
	//t.Execute(w, page)
}
...
func GetPageBooks(w http.ResponseWriter, r *http.Request) {
	...
	page, _ := dao.GetPageBooks(pageNo)
	//解析模板文件
	t := template.Must(template.ParseFiles("src/bookstore0612/views/pages/manager/book_manager.html"))
	//执行
	t.Execute(w, page)
}
...
func ToUpdateBookPage(w http.ResponseWriter, r *http.Request) {
	...
	if book.ID > 0 {
		//在更新图书
		//解析模板
		t := template.Must(template.ParseFiles("src/bookstore0612/views/pages/manager/book_edit.html"))
		//执行
		t.Execute(w, book)
	} else {
		//在添加图书
